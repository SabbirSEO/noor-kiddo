...
DIRS = ['arabic', 'bangla', 'english', 'numbers', 'duas', 'prayers', 'nature', 'ui']

for d in DIRS:
    os.makedirs(os.path.join(BASE_DIR, d), exist_ok=True)

# Voice Definitions
VOICE_BN = 'bn-BD-NabanitaNeural'
VOICE_BN_MALE = 'bn-BD-PradeepNeural'
...
ARABIC_DATA = [
    ...
    ('ar-25', 'ن', 'নূন', 'نُور', 'নূর', 'ঈমানের পবিত্র নূর ও আলো'),
...
NATURE_DATA = [
    ...
    ('nat-10', 'বেদানা বা ডালিম। জান্নাতের সুস্বাদু লাল টকটকে ডালিম।', 'Delicious pomegranate of Jannah.'),
...
async def main():
    ...
    for item in ARABIC_DATA:
        ...
        text_bn = f"{name_bn}। {word_bn}—{meaning}।"
        tasks.append(generate_file(text_bn, VOICE_BN, f"assets/audio/arabic/{aid}.mp3"))
        
    # 2. Bangla files
    for item in BANGLA_DATA:
        bid, char, name_bn, meaning = item
        text_bn = f"{char}। {name_bn}—{meaning}।"
        tasks.append(generate_file(text_bn, VOICE_BN, f"assets/audio/bangla/{bid}.mp3"))

    # 3. English files
    for item in ENGLISH_DATA:
        eid, char, name_en, meaning = item
        text_en = f"{char}, for {name_en}. {meaning}."
        tasks.append(generate_file(text_en, VOICE_EN, f"assets/audio/english/{eid}.mp3"))

    # 4. Numbers
    for item in NUMBERS_DATA:
        num, w_bn, m_bn, w_en, m_en, w_ar = item
        # Bangla
        tasks.append(generate_file(f"{w_bn}। {m_bn}।", VOICE_BN, f"assets/audio/numbers/num-bn-{num}.mp3"))
        # English
        tasks.append(generate_file(f"{w_en}. {m_en}.", VOICE_EN, f"assets/audio/numbers/num-en-{num}.mp3"))
        # Arabic
        tasks.append(generate_file(f"{w_ar}", VOICE_AR, f"assets/audio/numbers/num-ar-{num}.mp3"))

    # 5. Duas
    for item in DUAS_DATA:
        did, text_bn, text_en = item
        tasks.append(generate_file(text_bn, VOICE_BN, f"assets/audio/duas/{did}.mp3"))

    # 6. Prayers & Pillars
    for item in PRAYERS_DATA:
        pid, text_bn, text_en = item
        tasks.append(generate_file(text_bn, VOICE_BN, f"assets/audio/prayers/{pid}.mp3"))

    # 7. Nature
    for item in NATURE_DATA:
        nid, text_bn, text_en = item
        tasks.append(generate_file(text_bn, VOICE_BN, f"assets/audio/nature/{nid}.mp3"))

    # Execute tasks in batches of 10 to avoid connection overload
    batch_size = 10
